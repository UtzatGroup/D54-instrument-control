...
def createScanPoints(x_start, y_start, nx_pix, ny_pix, x_end, y_end, file_name, square_raster = False):
    ...
    filenameX = os.path.join(script_directory, file_name + "X.txt") 
    filenameY = os.path.join(script_directory, file_name + "Y.txt")

    motion_arrayY = np.zeros((ny_pix,nx_pix))  
    motion_arrayX = np.zeros((ny_pix,nx_pix))  
    for y in range(ny_pix):    
        for x in range(nx_pix):
            motion_arrayY[y,x] = (y_end-y_start)/(ny_pix)*y+y_start

    motion_listY = motion_arrayY.reshape(1,-1)
    for y in range(ny_pix):    
        for x in range(nx_pix):
            motion_arrayX[y,x] = (x_end-x_start)/nx_pix*x+x_start
...
def startScanningWithoutWaveform(fileX = None , fileY = None, fileZ = None, dwell_time= 0, iterations = 1):
    ...
    for j in range(iterations):
        for i in range(len(waveformX)):
            ...
            print(mcldll.MCL_SingleWriteN(posX, axisX, handle))
            mcldll.MCL_SingleWriteN(posY, axisY, handle)
            print(mcldll.MCL_SingleReadN(axisX, handle))
...
'''
startScanningWithoutWaveform('pathX.txt' , 'pathY.txt', None, 100000, 1)
...
createScanPoints(x_start = x_start, y_start = y_start, nx_pix = nx_pix, ny_pix = ny_pix, x_end = x_end, y_end =  200, file_name = 'path', square_raster = False)
'''
'''
...
'''
====================================================================================================================
this code allows you to control the nanodrive stage with the keyboard
====================================================================================================================
'''
'''
handle = mcldll.MCL_InitHandle()
...
def move_positive_x():
    ...
    mcldll.MCL_SingleWriteN(posX, axisX, handle)
    sleep(.1)
    X_position = mcldll.MCL_SingleReadN(axisX, handle)
    print(f'X Stage Position is {X_position}')
...
def move_positive_y():
    ...
    mcldll.MCL_SingleWriteN(posY, axisY, handle)
    sleep(.1)
    Y_position = mcldll.MCL_SingleReadN(axisY, handle)
    print(f'Y Stage Position is {Y_position}')
...
def move_positive_z():
    ...
    i_z += .5
    posZ= c_double(i_z)
    mcldll.MCL_SingleWriteN(posZ, axisZ, handle)
    sleep(.1)
    Z_position = mcldll.MCL_SingleReadN(axisZ, handle)
    print(f'Z Stage Position is {Z_position}')
